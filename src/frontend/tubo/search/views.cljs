...
(defn search-form
  ...
  (let [service-query     @(rf/subscribe [:search/query])
    ...
    (.log js/console (clj->js suggestions))
    [:<>
     ...
     (when (and (seq suggestions) show-suggestions?)
       [:div.bg-neutral-800.min-h-full.py-2.md:my-2.md:rounded.w-full.lg:w-96.flex.flex-col.absolute.top-full
        {:class ["md:w-11/12" "-translate-x-1/2" "left-1/2"]}
        (for [[i suggestion] (map-indexed vector suggestions)]
          ^{:key i}
          [:div.px-4.py-2.hover:bg-neutral-700.cursor-pointer.flex.justify-between.items-center.gap-x-4
